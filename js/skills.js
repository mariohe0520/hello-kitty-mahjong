...
const Skills = (() => {
  ...
  function showSkillActivation(skill, result) {
    ...
    if (result.type === 'peek_wall' && result.tiles.length > 0) {
      ...
      Commentary.announce('安全结界启动！3回合内可以看到所有牌的危险度！', '#9b59b6');
    } else if (result.type === 'double_draw') {
      Commentary.announce('暴力摸牌！下次摸牌时可以选择最好的一张！', '#8B4513');
    } else if (result.type === 'best_draw') {
      Commentary.announce('龙之召唤！下次将摸到你最需要的牌！', '#c0392b');
    }
  }
  ...
  function handleSwapSelection(tile, gameState) {
    ...
    Commentary.announce(`偷天换日！获得了 ${wallTile.name}！`, '#e67e22');
    renderSkillButton();
    return true;
  }
